...
async function fetchSequentialAyah(isNext, endeed = false) {
    if (audio) {
        ...
        const newQuranAyah = `https://api.alquran.cloud/v1/ayah/${newAyahNumber}/ar.alafasy`;
...
const TAFSEER_API_URL = 'http://api.quran-tafseer.com/tafseer';

// Function to fetch and display tafseer for the current ayah
async function getAyahTafseer(surah, ayah) {
    const tafseerUrl = `${TAFSEER_API_URL}/1/${surah}/${ayah}`;

    try {
        const proxyUrl = `${CORS_PROXY}?${tafseerUrl}`;
        const tafseer = await fetchData(proxyUrl);
